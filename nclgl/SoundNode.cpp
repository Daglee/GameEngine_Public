...
SoundNode::SoundNode(Sound* s, SceneNode* node) {
	sceneNode = node;
...
void		SoundNode::UpdateSoundState(float msec) {
	if(sound) {
		...
		if(oalSource) {
			...
			if(isGlobal) {
				position = SoundSystem::GetSoundSystem()->GetListener()->GetTransform().GetPositionVector();
			}
			else{
				position = sceneNode->GetTransform().GetPositionVector();
			}
