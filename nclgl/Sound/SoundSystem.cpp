...
SoundSystem::SoundSystem(bool verbosity, unsigned int channels) {
	listener		= NULL;
	masterVolume	= 1.0f;
	verbose			= verbosity;

	if (verbose) cout << "Creating SoundSystem!" << endl;
	if (verbose) cout << "Found the following devices: " << alcGetString(NULL,ALC_DEVICE_SPECIFIER) << endl;	//outputs all OAL devices

	device = alcOpenDevice(NULL);	//Open the 'best' device

	if(!device && verbose) cout << "Failed to create SoundSystem! (No valid device!)" << endl;

	if (verbose) cout << "SoundSystem created with device: " << alcGetString(device,ALC_DEVICE_SPECIFIER) << endl;	//Outputs used OAL device!
	...
	if (verbose) cout << "SoundSystem has " << sources.size() << " channels available!"  << endl;
}
...
void	SoundSystem::CullNodes() {
	for(vector<SoundNode*>::iterator i = emitters.begin(); i != emitters.end();) {
		...
		if((*i)->GetIsGlobal()) {
			...
			length = (listener->GetTransform().GetPositionVector() - 
				(*i)->sceneNode->GetTransform().GetPositionVector()).Length();
		}
...
void	SoundSystem::UpdateListener() {
	if(listener)	{
		Matrix4 worldMat = listener->GetTransform();
...
void	SoundSystem::PlaySound(Sound* s, Vector3 position) {
	...
	n->sceneNode = new SceneNode();
	n->sceneNode->SetTransform(Matrix4::Translation(position));
