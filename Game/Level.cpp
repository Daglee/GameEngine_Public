...
void Level::UnloadItem(std::string resourcemanager, std::string resourcename) 
{
	if (resourcemanager == "OBJMeshes") {
		...
		database->GPhysicsEngine->Find("PhysicsEngine")->RemoveRigidBody(rigidBody);
	...
	for (int i = 0; i < numItems; ++i) {
		...
		while (getline(file, line)) {
			...
			if (variable == "size") {
				...
				Vector3 rotation(stof(tokens.at(2)), stof(tokens.at(3)), stof(tokens.at(4)));

				obj->GetSceneNode()->SetTransform(obj->GetSceneNode()->GetTransform() *
					Matrix4::Rotation(stof(tokens.at(1)), rotation));
			}
