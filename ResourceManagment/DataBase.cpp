...
//Just a style choice.
#define MAX_MEM_PER_TYPE 5000
#define MAX_MEM_PLAYERS 7000
#define MAX_MEM_OBJMESHES 9000
#define MAX_MEM_PHYSOBJ 15000
...
DataBase::DataBase()
{
	...
	Meshes			= new ResourceManager<Mesh>			(false, true);
...
void DataBase::InitialiseDataBase()
{
	...
	Meshes			->Initialise("Mesh Manager",			size_t(MAX_MEM_OBJMESHES),	OBJMESH_BINS);
...
void DataBase::InitialiseFunctionMap()
{
	...
	*/
	...
	addmap["Meshes"] = &DataBase::AddToMeshes;
}
...
{
	PhysicsEngine* physics = new PhysicsEngine(GRenderer->Find("Renderer"), this);
...
void DataBase::AddToMeshes(string meshLocation)
{
	Mesh* mesh = new Mesh();
	if (meshLocation == "quad") {
		mesh = mesh->GenerateQuad();
	}
	Meshes->Load(meshLocation, mesh);
}
